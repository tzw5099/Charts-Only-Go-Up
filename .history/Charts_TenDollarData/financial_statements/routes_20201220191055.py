...
def current_ratio(url_fin_metric,stock_or_etf,url_name,statement_or_ratio,url_symbol): 
    ...
    if "{}".format(statement_or_ratio) in fin_statements_list:
        ...
        df_tall['pct_chg'] = df_tall['{}'.format(fin_metric_title)]/df_tall['{}'.format(fin_metric_title)].shift(-4)
        df_tall['pct_chg'] = df_tall['pct_chg'].apply(lambda x: "{}%".format(round((x-1)*100,1)))
        ...
        df_tall['pct_chg'] = df_tall['{}'.format(fin_metric_name)]/df_tall['{}'.format(fin_metric_name)].shift(-4)
        df_tall['pct_chg'] = df_tall['pct_chg'].apply(lambda x: "{}%".format(round((x-1)*100,1)))
        df_html_tall = df_tall[['{}'.format('Quarter & Year'),'{}'.format(fin_metric_name), 'pct_chg']].to_html(index=False)
        df_html_tall.columns = ["Quarter & Year", "{}".format(fin_metric_title),"pct_chg"]
