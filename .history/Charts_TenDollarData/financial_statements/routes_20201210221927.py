...
urilist = ["/log", "/current_ratio", "/loginexample"]
@charts.route('<urllist>/current_ratio/<some_place>', methods=['POST', 'GET'])
# @cache.cached(timeout=5)
def current_ratio(some_place):

    start_time = time.time()
    csv_file = glob.glob("Charts_TenDollarData/financial_statements/data/Historical Financial Statements/*/year/Income Statement/*_{}_*".format(some_place))[-1]
    ...
    return render_template('current_ratio.html',
     df_date = df['date'].to_list(), df_rev = df['revenue'].to_list(),
       =df.to_numpy().tolist(), table_pct = [df_pct], tables=[df_html], titles=df.columns.values,
