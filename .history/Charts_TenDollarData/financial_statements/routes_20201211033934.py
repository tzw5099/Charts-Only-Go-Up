...
@charts.route('/<urilist>/<some_place>', methods=['POST', 'GET'])
# @cache.cached(timeout=5)
def current_ratio(urilist,some_place):
    ...
    start_time = time.time()
    csv_file = glob.glob("Charts_TenDollarData/financial_statements/data/Historical Financial Statements/*/year/{}/*~{}~*".format(fin_statement_dir, some_place))[-1]
    ...
    if fin_metric_history[0]  > billion:
        print((fin_metric_history/billion).round(decimals=2))
        df_bs_table_html = df_bs[['{}'.format(fin_metric_name)]].round(decimals=2).transpose().to_html().replace("'","")
    ...
    return render_template('current_ratio.html', df_bs_table_html = [df_bs_table_html], fin_metric_name = fin_metric_name,
