...
def current_ratio(url_fin_metric,stock_or_etf,url_name,statement_or_ratio,url_symbol): # WORKS
    ...
    df_fin_statement = df_fin_statement.drop(['Quarter & Year'])
    df_fin_statement['Quarter & Year'] =(df_fin_statement['date'].astype(str).str[0:4]).astype(int)
    df_fin_statement = df_fin_statement.drop(['Quarter & Year', 'Unnamed: 0','date','symbol','fillingDate','acceptedDate','period','link'],axis=1, errors='ignore')
