...
def current_ratio(url_fin_metric,stock_or_etf,url_name,statement_or_ratio,url_symbol): 
    ...
    if "{}".format(statement_or_ratio) in fin_statements_list:
        ...
        filter_pos_neg = list(fin_statements_matching[fin_statements_matching['Name']=="{}".format(fin_metric_name)]['positive_negative'])[0]
        
        filter_pos_neg_list = ['all', 'both', 'negative', 'zero_or_neg', 'zero_or_pos']
        if "{}".format(filter_pos_neg) in filter_pos_neg_list:
            filter_pos_neg_list
        ...
        if "{}".format(filter_pos_neg) == "positive":
            ...
            pass
        df["{}".format(fin_metric_name)] = df_metric.fillna(df_metric.rolling(4,center=True,min_periods=1).mean())
        ...
        year_df = df.groupby("Year").sum()
        sorted_metric = year_df.sort_values(ascending=True, by="Year")
