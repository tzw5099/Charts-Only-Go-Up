...
def current_ratio(url_fin_metric,stock_or_etf,url_name,statement_or_ratio,url_symbol): # WORKS
    ...
    if "{}".format(statement_or_ratio) in fin_statements_list:
        ...
        df["QQ-YYYY"] = df["period"]+"-"+df["date"].apply(lambda x: x[-4:])#["QQ-YYYY"]
        ...
        for x in fin_dir:
            ...
            fin_df["QQ-YYYY"] = fin_df["period"]+"-"+fin_df["date"].apply(lambda x: x[-4:])#["QQ-YYYY"]
            print("this is qq yyyy", fin_df["QQ-YYYY"])
    ...
    print("latest num", earliest_metric,latest_metric)
    ...
    annual_pct_chg  = (latest_metric/earliest_metric)/2# round(pct_chg**(1/num_years),2)
    ...
    # df_tall['pct_chg'] = df_tall.apply(lambda x: x)
    if "{}".format(statement_or_ratio) in fin_statements_list:

        
        df_html_tall = df_tall[['{}'.format('QQ-YYYY'),'{}'.format(fin_metric_title),'pct_chg']].to_html(index=False)
