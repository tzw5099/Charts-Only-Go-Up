...
def current_ratio(url_fin_metric,stock_or_etf,url_name,statement_or_ratio,url_symbol): # WORKS
    ...
    pct_chg = 1+(latest_metric/earliest_metric) #1+((latest_metric - earliest_metric)/earliest_metric)
    ...
    annual_pct_chg  = round(pct_chg**(1/num_years),2)
    ...
    if pct_chg>=0:
        ...
        annual_pct_chg_str = "{}%".format(annual_pct_chg*100-1)
    elif pct_chg<0:
        hist_pct_chg_str = "-{}x".format(historical_pct_chg)
        annual_pct_chg_str = "-{}%".format(annual_pct_chg*100-1)
    else:
