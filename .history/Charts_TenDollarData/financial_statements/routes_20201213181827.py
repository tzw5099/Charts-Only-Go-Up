...
# def current_ratio(urilist,url_symbol_name):
    ...
    if list(fin_metric_history)[0]  > billion:
        ...
        df_ = df_tall[['{}'.format(fin_metric_name)]].to_html().replace("'","")
        # df_html_tall = df_html_tall.replace("\n","")
        df_html_tall = df_html_tall.replace("\n","")
        df_html_tall = df_html_tall.replace("{}".format("["),"")
        df_html_tall = render_template_string(df_html_tall)
    ...
    return render_template('current_ratio.html', company_symbol = profiles_dict['symbol'],\
                            ...
                            max_min_pct_diff_str = max_min_pct_diff_str, df_bs_table_html = [df_table_html].replace("n",""),df_html_tall = [df_],fin_metric_name = fin_metric_title,\
