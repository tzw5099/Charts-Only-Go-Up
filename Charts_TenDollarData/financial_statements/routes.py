...
def current_ratio(url_fin_metric,stock_or_etf,url_name,statement_or_ratio,url_symbol):
    ...
    titles_list = ['Selling, General and Administrative (SG&A)','Selling General and Administrative (SG&A)']
    vars_drop = ['quarter_n_year',  's_g_n_a',]
    ...
    num_years_increased = if_error_value(len(metric_increased_years)+1,0, metric_increased_years)
    num_years_bad = if_error_value(num_years - num_years_increased,0, metric_increased_years)
    price_increased_on_metric_up_years = if_error_value(len(metric_increased_years[metric_increased_years['YoY Price % Change float']>1]),0, metric_increased_years)
    percent_correlation = weird_division(price_increased_on_metric_up_years*100,num_years_increased)
    percent_correlation = if_error_value(percent_correlation,0, metric_increased_years)
